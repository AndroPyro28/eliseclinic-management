...
export default function DashboardModule() {
  ...
  const { result: patientsResult, isLoading: patientsLoading } = useFetch(() =>
  ...
  const recentPatientsDataColumns = [
    {
  ...
  const recentInvoiceDataColumns = recentPatientsDataColumns.map((column) => {
    if (column.title === translate('Patient')) {
      return {
        ...column,
        dataIndex: 'patient',
      };
    ...
    if (column.title === translate('number')) {
      return {
        ...column,
  ...
  const entityData = [
    ...
    },
    {
      ...
      entity: 'offer',
      title: translate('Suppliers preview'),
    },
    // {
  ...
  const cards = entityData.map((data, index) => {
    ...
    return (
      <SummaryCard
        key={index}
        title={data?.entity === 'offer' ? translate('Offer') : translate(data?.entity)}
        tagColor={
      ...
      <Row gutter={[32, 32]}>
        <Col className="gutter-row w-full" sm={{ span: 24 }} lg={{ span: 12 }}>
          <div className="whiteBox shadow pad20" style={{ height: '100%' }}>
            ...
            <RecentTable entity={'invoice'} dataTableColumns={recentInvoiceDataColumns} />
          </div>
        </Col>
        ...
        <Col className="gutter-row w-full" sm={{ span: 24 }} lg={{ span: 12 }}>
          <div className="whiteBox shadow pad20" style={{ height: '100%' }}>
            ...
            <RecentTable entity={'patients'} dataTableColumns={recentPatientsDataColumns} />
          </div>
        </Col>
      </Row>
    </>
