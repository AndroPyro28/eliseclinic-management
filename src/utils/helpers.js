...
*/
export function valueByString(obj, string, devider, objProp) {
  ...
  return string
    ...
    .map(function (key) {
      if (objProp) {
        if (objProp === 'fullName') {
          return `${obj.patient['firstName']} ${obj.patient['lastName']}`;
        }

        const innerObj = get(obj, key);
        return get(innerObj, objProp);
      }
      return get(obj, key);
    })
    .join(' ');
